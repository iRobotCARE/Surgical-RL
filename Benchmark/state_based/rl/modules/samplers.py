from rl.utils.general_utils import AttrDict, listdict2dictlist
from rl.utils.rl_utils import ReplayCache
import numpy as np
...
class Sampler:
    ...
    def _safe_render(self, mode='rgb_array'):
        """
        Call env.render in a robust way to support different env implementations/wrappers.
        Try keyword mode first, then positional, then no-arg.
        """
        try:
    ...
    def _parse_step_result(self, step_result):
        """
        Normalize env.step(...) return value to (obs, reward, done, info).
        Supports:
          - gymnasium 5-tuple: (obs, reward, terminated, truncated, info)
          - old gym 4-tuple: (obs, reward, done, info)
        Also unwraps observation if it's returned as (obs, info) tuple.
        """
        if not isinstance(step_result, tuple) and not isinstance(step_result, list):
            raise ValueError(f"Unexpected env.step() return type: {type(step_result)}")
        ...
        # If obs itself is (obs, info), unwrap it for the agent/cache convenience
    ...
    def sample_episode(self, is_train, render=False, random_act=False):
        ...
        while not done and self._episode_step < self._max_episode_len:
            ...
            if action is None:
                break
            if render:
                render_obs = self._safe_render('rgb_array')
            step_result = self._env.step(action)
            obs, reward, done, info = self._parse_step_result(step_result)
...
class HierarchicalSampler(Sampler):
    ...
    def sample_episode(self, is_train, render=False):
        ...
        while not done and self._episode_step < self._max_episode_len:
            ...
            step_result = self._env.step(agent_output.sl_action)
            obs, reward, done, info = self._parse_step_result(step_result)
            self.reward_since_last_sc += reward
            if info.get('subtask_done', False):
                if not done:
                    ...
                    if info.get('subtask_is_success', False):
                ...
                prev_subtask_succ[self._env.subtask] = info.get('subtask_is_success', False)
            else:
