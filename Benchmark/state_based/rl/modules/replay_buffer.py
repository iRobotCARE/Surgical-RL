...
def _get_wrapped_attr(env, name):
    """
    Try to find attribute `name` in env or any wrapped inner env.
    Returns the attribute (callable or value) if found, otherwise None.
    """
    # direct attribute
    if hasattr(env, name):
        return getattr(env, name)
    # try unwrapped (gym/gymnasium)
    try:
        inner = getattr(env, "unwrapped", None)
    except Exception:
        inner = None
    if inner is not None and inner is not env and hasattr(inner, name):
...
def get_buffer_sampler(env, cfg):
    # find compute_reward on wrapped envs if necessary
    reward_func = _get_wrapped_attr(env, "compute_reward")
    if reward_func is None:
        raise AttributeError(
            "Could not find 'compute_reward' on env or any wrapped inner env. "
            "Ensure your base environment implements compute_reward or pass the unwrapped env "
            "(env.unwrapped) to get_buffer_sampler."
        )
...
def get_hier_buffer_samplers(env, cfg):
    # try to find get_reward_functions on wrapped envs if necessary
    get_reward_functions = _get_wrapped_attr(env, "get_reward_functions")
    if get_reward_functions is None:
        raise AttributeError(
            "Could not find 'get_reward_functions' on env or any wrapped inner env. "
            "Ensure your base environment implements get_reward_functions or pass the unwrapped env "
            "(env.unwrapped) to get_hier_buffer_samplers."
        )
