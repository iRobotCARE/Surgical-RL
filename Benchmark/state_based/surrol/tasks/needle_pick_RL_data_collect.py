...
env_setup_counter = 0       # 环境被重置的次数, 全局变量
...
class NeedlePickRL(PsmEnv):
    ...
    def get_oracle_action(self, obs) -> np.ndarray:
        ...
        # 1. 定义相对于仓库根目录的数据保存基础路径
        # 这个路径是 'root/Benchmark/state_based/surrol/data'
        base_save_path = 'Benchmark/state_based/surrol/data/experiment_data/needlepick'

        # 2. 构造本次实验具体的数据保存目录
        experiment_data_dir = os.path.join(base_save_path, str(env_setup_counter))
        ...
        torch.save(self.observation_info, os.path.join(experiment_data_dir, 'observation.pth'))
        torch.save(self.goal_info, os.path.join(experiment_data_dir, 'goal.pth'))
        torch.save(self.action_info, os.path.join(experiment_data_dir, 'action.pth'))

        # print(f"数据已成功保存到: {os.path.abspath(experiment_data_dir)}")
