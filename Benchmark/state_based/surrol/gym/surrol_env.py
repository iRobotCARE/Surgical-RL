...
class SurRoLEnv(gym.Env):
    """
    A gymnasium.Env wrapper for SurRoL.
    ...
    def __init__(self, render_mode: str = None, cid: int = -1):
        ...
        # initialize RNG (no seed passed -> random)
        self.seed()
    ...
    def step(self, action: np.ndarray):
        ...
        # No termination logic here by default; user tasks can override
        terminated = False
        truncated = False
        ...
        # Return gymnasium-compatible 5-tuple
    ...
    def reset(self, *, seed: int = None, options: dict = None):
        ...
        Accepts gymnasium-style keyword arguments: seed and options.
        Returns (obs, info) per gymnasium API.
        """
        # set RNG if seed provided
        if seed is not None:
            # keep same seeding implementation
            self.seed(seed)
        ...
        obs = self._get_obs()
        info = {}  # you can add diagnostics here if needed
        return obs, info
    ...
    def seed(self, seed=None):
        # Keep existing behavior using gymnasium seeding util.
        # Returns a list like the original gym implementation for compatibility with code that expects that.
    ...
    def test(self, horizon=200):
        ...
        while not done and steps <= 9999900:
            ...
            print(f"Rob pose: {np.round(obs['observation'][:7], 4)}")
            # print('action:', action)
            # Unpack gymnasium 5-tuple
            obs, reward, terminated, truncated, info = self.step(action)
            if isinstance(obs, dict):
                print(" -> achieved goal: {}".format(np.round(obs['achieved_goal'], 4)))
                print(" -> desired goal: {}".format(np.round(obs['desired_goal'], 4)))
            else:
                print(" -> achieved goal: {}".format(np.round(info.get('achieved_goal', None), 4)))
            # Determine done using is_success if available, or terminated/truncated
            done = info.get('is_success', False) or terminated or truncated
            steps += 1
            toc = time.time()
