...
class BETLowdimPolicy(BaseLowdimPolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        B, _, Do = nobs.shape   # B: batch size, Do: obs dim
        To = self.n_obs_steps   # observation horizon
        T = self.horizon        # 

        # pad To to T, since may To < T
        obs = torch.full((B,T,Do), -2, dtype=nobs.dtype, device=nobs.device)
        obs[:,:To,:] = nobs[:,:To,:]    # 将To放在T的最前面

        # (B,T,Do)
        enc_obs = self.obs_encoding_net(obs)    # 相当于词嵌入
