...
class TrainBETLowdimWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf):
        ...
        # 1. First, create the environment runner.
        # This ensures PyBullet/EGL initializes BEFORE PyTorch touches the GPU.
        print("Initializing environment runner first to prevent CUDA/EGL conflict...")
        self.env_runner: BaseLowdimRunner = hydra.utils.instantiate(
            cfg.task.env_runner,
            output_dir=self.output_dir
        )
        print("Environment runner created successfully.")

        # configure model
        self.policy: BETLowdimPolicy
        self.policy = hydra.utils.instantiate(cfg.policy)       # 从cfg中获取模型参数
    ...
    def run(self):
        ...
        if cfg.training.enable_normalizer:
            normalizer = dataset.get_normalizer()       # 没看到定义
        else:
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                # ========= eval for this epoch ==========
                self.policy.eval()      # set eval mode, disable dropout and batchnorm
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        val_losses = list()
                        # tepoch作为一个迭代器，相当于val_dataloader
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        if cfg.pred_action_steps_only:
                            ...
                            end = start + cfg.n_action_steps    # 应该是评估一个预测区间的difference
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0:
                    ...
                    if cfg.checkpoint.save_last_ckpt:
                        self.save_checkpoint()      # 只保存最新的一个checkpoint
